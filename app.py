...
def realizar_request(url, params=None):
...
def pegar_nome_por_decada(nome):
    ...
    nome_por_decada = realizar_request(url=url)
...
def main():
    ...
    if not dict_decadas:  # Sem dados do IBGE para para pesquisa
        st.warning(f'Nenhum dado encontrado para o nome "{nome}"!')
        st.stop()

    # Mostra dados do IBGE
